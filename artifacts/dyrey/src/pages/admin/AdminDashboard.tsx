...
function useScheduleCapacity() {
  return useQuery<CapacityRow[]>({
    ...
    queryFn: async () => {
      const token = localStorage.getItem("admin_token") ?? "";
      const res = await fetch(`${BASE}/api/admin/schedule-capacity`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error("Failed");
      return res.json();
...
function CapacityEditor({ date, current, onSaved }: { date: string; current: number; onSaved: () => void }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-1 text-xs text-slate-400 hover:text-primary mt-1 mx-auto"
      title="Set max concurrent bookings"
    >
...
export default function AdminDashboard() {
  ...
  const pieData = Object.entries(analytics?.statusCount ?? {})
    .filter(([, v]) => v > 0)
      ...
      <div className="space-y-8 max-w-6xl">
              ...
              <CardContent className="flex flex-col items-center">
                {pieData.length === 0 ? (
                  ...
                  <PieChart width={180} height={180}>
                    <Pie data={pieData} cx={90} cy={90} innerRadius={52} outerRadius={80} dataKey="value" paddingAngle={2}>
                      {pieData.map((entry, i) => (
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1">
                  ...
                  <Button size="sm" variant="outline" className="h-7 text-xs px-2" onClick={() => setWeekOffset(0)}>
                    Today
                  </Button>
                  <Button size="icon" variant="outline" className="h-7 w-7" onClick={() => setWeekOffset(w => w + 1)}>
            ...
            <p className="text-xs text-slate-400 mt-1">Click the pencil icon on each day to set max concurrent bookings for that day.</p>
          </CardHeader>
                  ...
                  {weekDays.map(day => {
                    ...
                    return (
                      <th key={dateStr} className={`p-2 text-center border-b border-r font-normal ${isToday ? "bg-primary/5" : "bg-slate-50/50"}`}>
                        <div className={`font-semibold ${isToday ? "text-primary" : "text-slate-700"}`}>
                          {format(day, "EEE")}
                        </div>
                        <div className={`text-slate-500 ${isToday ? "font-bold text-primary" : ""}`}>
                          {format(day, "d MMM")}
                        </div>
                        <CapacityEditor
                          date={dateStr}
                          current={cap}
                          onSaved={() => queryClient.invalidateQueries({ queryKey: ["admin", "schedule-capacity"] })}
                        />
                      </th>
                    );
                  })}
                        ...
                        <td key={dateStr} className={`p-1.5 border-r align-top min-w-[90px] ${isToday ? "bg-primary/5" : ""} ${isOver ? "bg-red-50" : ""}`}>
                          {aptsInSlot.length > 0 && (
                            <div className="space-y-1">
                              ...
                              {isOver && (
                                <div className="text-red-500 font-semibold">{aptsInSlot.length}/{cap} ⚠</div>
                              )}
                            </div>
                          )}
                        </td>
